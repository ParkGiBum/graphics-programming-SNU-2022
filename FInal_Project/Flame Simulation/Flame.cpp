...
Flame::Flame(Material* _material, unsigned int* _deltaTime)
{
	...
	m_accBuffer = 10;
	std::fill_n(m_accpositions, NUMBEROFPARTICLES, glm::vec3(0));
	glBindBuffer(GL_ARRAY_BUFFER, m_posBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * NUMBEROFPARTICLES , &m_accpositions[0], GL_STATIC_DRAW);
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3, m_posBuffer);
	//glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
	 
	m_numParticles = 0;
	//create the first particle
	m_particles.push_back(new Particle(1));
}
...
void Flame::Update(bool pause)
{
	...
	if (m_particles.size() < NUMBEROFPARTICLES - 1 )
	{
		if (NUMBEROFPARTICLES - m_particles.size() < 10) {
			for (int i = 0; i < (NUMBEROFPARTICLES - m_particles.size()); i++)
			{
				int v1 = rand() % 100;
				if(v1 < 1)
					m_particles.push_back(new Particle(2));
				else
					m_particles.push_back(new Particle(1));
			}
	...
	if (first) {
		first = false;
		//glBindBuffer(GL_ARRAY_BUFFER, m_accBuffer);
		//glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * NUMBEROFPARTICLES * 3, &m_accpositions[0]);
	}
	glBindBuffer(GL_ARRAY_BUFFER, m_accBuffer);
